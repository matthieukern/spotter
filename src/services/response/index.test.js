...
describe('author', () => {
  ...
  it('returns the passed entity when author is the same', () => {
    expect(response.author(res, user, 'author')(entity)).toEqual(entity);
  });

  it('responds with status 401 when author is not the same', () => {
    user.id = 2;
    expect(response.author(res, user, 'author')(entity)).toBeNull();
    expect(res.status).toBeCalledWith(401);
    expect(res.end).toHaveBeenCalledTimes(1);
  });

  it('returns null without sending response when entity has not been passed', () => {
    expect(response.author(res, user, 'author')()).toBeNull();
  });
});
